#!/usr/bin/env python3
"""Create import scripts for websites - all states"""
...
"""Update database with verified {state_name} websites from CSV"""
...
def update_websites_from_csv(csv_file: str, confidence: float = 1.0, source: str = 'manual_verification'):
    ...
    print("════════════════════════════════════════════════════════════════════")
    print("UPDATE {state_upper} WEBSITES FROM CSV")
    print("════════════════════════════════════════════════════════════════════\\n")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python update_{state_lower}_websites_from_csv.py <csv_file> [confidence] [source]")
        print("\\nExample:")
        print("  python update_{state_lower}_websites_from_csv.py {state_lower}_website_lookup.csv 1.0 manual_verification")
        sys.exit(1)
...
def main():
    ...
    print("════════════════════════════════════════════════════════════════════")
    print("CREATE WEBSITE IMPORT SCRIPTS FOR ALL STATES")
    print("════════════════════════════════════════════════════════════════════\n")

    scripts_dir = Path('scripts')

    for state_code, state_name in STATES.items():
        filename = f'update_{state_code.lower()}_websites_from_csv.py'
    ...
    print(f"\n✅ Created {len(STATES)} website import scripts!\n")
    print("Scripts ready:")
    for state_code in STATES.keys():
        print(f"  • update_{state_code.lower()}_websites_from_csv.py")
