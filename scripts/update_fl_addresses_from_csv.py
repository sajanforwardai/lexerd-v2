#!/usr/bin/env python3
"""Update database with verified Florida addresses from CSV"""
...
def update_from_csv(csv_file: str, confidence: float = 1.0, source: str = 'manual_verification'):
    ...
    Args:
        csv_file: Path to CSV file with columns:
                 Property Name, City, County, Address (to be filled), Confidence
        confidence: Confidence score for imported addresses (default 1.0 for manual)
        source: Source label ('manual_verification', 'county_assessor', etc.)
    """

    print("════════════════════════════════════════════════════════════════════")
    print("UPDATE FLORIDA ADDRESSES FROM CSV")
    print("════════════════════════════════════════════════════════════════════\n")
    ...
    for row in with_addresses:
        ...
        loans = search_loans(session, property_name=property_name, city=city, state='FL')
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python update_fl_addresses_from_csv.py <csv_file> [confidence] [source]")
        print("\nExample:")
        print("  python update_fl_addresses_from_csv.py fl_property_lookup.csv 1.0 manual_verification")
        print("\nCSV Format:")
        print("  Property Name | City | County | Address (to be filled) | Confidence")
        print("\nDefault:")
        print("  confidence=1.0 (manual verification)")
        print("  source='manual_verification'")
        sys.exit(1)
