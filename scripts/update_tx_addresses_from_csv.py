#!/usr/bin/env python3
"""Update database with verified Texas addresses from CSV"""
...
def update_from_csv(csv_file: str, confidence: float = 1.0, source: str = 'manual_verification'):
    ...
    print("════════════════════════════════════════════════════════════════════")
    print("UPDATE TEXAS ADDRESSES FROM CSV")
    print("════════════════════════════════════════════════════════════════════\n")
    ...
    for row in with_addresses:
        ...
        loans = search_loans(session, property_name=property_name, city=city, state='TX')
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python update_tx_addresses_from_csv.py <csv_file> [confidence] [source]")
        print("\nExample:")
        print("  python update_tx_addresses_from_csv.py tx_property_lookup.csv 1.0 manual_verification")
        print("\nCSV Format:")
